Contributing to Unitedcore
=======

We're working hard to make *unitedcore* the most powerful JavaScript library for working with unitedcoin. Our goal is to have *unitedcore* be a library that can be used by anyone interested in unitedcoin, and to level expertise differences with great design and documentation.
...
These are some global design goals in unitedcore that any change must adhere.
...
When possible, unitedcore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Unitedcore methods. For example, calling `JSON.stringify` on an Unitedcore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'unitedcore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the unitedcore object.

DO:
```javascript
var unitedcore = require('../');
var PublicKey = unitedcore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add coinshaz git@github.com:coinshaz/unitedcore.git
git pull --rebase coinshaz master
```
...
```
Finally go to [github.com/coinshaz/unitedcore](https://github.com/coinshaz/unitedcore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of unitedcore.

If you have any questions feel free to post them to
[github.com/coinshaz/unitedcore/issues](https://github.com/coinshaz/unitedcore/issues).
